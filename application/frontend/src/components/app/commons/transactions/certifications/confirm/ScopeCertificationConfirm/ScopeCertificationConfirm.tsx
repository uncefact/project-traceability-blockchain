...
import {SCOPE_CERTIFICATION_CONFIRMATION_PATH} from "../../../../../../../routes/Routes";
...
export const ScopeCertificationConfirm = (props: Props) => {
    const history = useHistory();
    // @ts-ignore
    const idParam = useRouteMatch('/:companyIndustry' + SCOPE_CERTIFICATION_CONFIRMATION_PATH)?.params.id;
    ...
    const handleConfirmation = async (isConfirmed: boolean) => {
        ...
        try {
            ...
            props.addSuccessMessage(t("popups.success.scope_cert_add"));
            history.push("/");
        } catch (error) {
    ...
    return (
        <Jumbotron className={styles.Container}>
            <h2>{t("certification.confirmation.scope_title")}</h2>
            <Form className={styles.Form}>
                ...
                <Form.Group className={styles.ProcessTypesArea}>
                    ...
                    <ListGroup variant="flush">
                        {
                            // @ts-ignore
                            props.certification?.processTypes?.length > 0 && props.certification.processTypes.map((c: string, index: number) => {
                                return <ListGroup.Item className={styles.List} disabled key={index}>{c}</ListGroup.Item>
                            })
                        }
                        {props.certification?.processTypes?.length === 0 &&
                        <ListGroup.Item className={styles.List} disabled>{t("certification.no_process_types")}</ListGroup.Item>}
                    </ListGroup>
                </Form.Group>
                <Form.Group className={styles.OutputMaterialsArea}>
                    <Form.Label>{`${t("materials")} (${"output"})`}</Form.Label>
                    <ListGroup variant="flush">
                        {
                            // @ts-ignore
                            props.certification?.outputMaterials?.length > 0 && props.certification.outputMaterials.map((m: MaterialPresentable, index: number) => {
                                return <ListGroup.Item className={styles.List} disabled key={index}>{m.name}</ListGroup.Item>
                            })
                        }
                        {props.certification?.outputMaterials?.length === 0 &&
                            <ListGroup.Item className={styles.List} disabled>{t("certification.no_output_materials")}</ListGroup.Item>}
                    </ListGroup>
                </Form.Group>
...
export default connector(ScopeCertificationConfirm);
