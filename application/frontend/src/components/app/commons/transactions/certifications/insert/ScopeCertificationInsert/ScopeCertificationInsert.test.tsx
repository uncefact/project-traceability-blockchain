...
import SCI, {ScopeCertificationInsert} from "./ScopeCertificationInsert";
...
describe('Material certification insert test', () => {
    ...
    it('Render without crashing async', async () => {
        ...
        await act(async () => {
            mount(<ScopeCertificationInsert
                selectCompany={selectCompany} companySelected={companySelected} consigneeEmailSelected={consigneeEmailSelected}
                getUserFromEmailAddress={getUserFromEmailAddress} companyEmailAddresses={companyEmailAddresses}
                getMaterialsByCompany={getMaterialsByCompany}
                checkValidUntilDate={checkValidUntilDate}
    ...
    it('Content test', async () => {
        ...
        const outputMaterials = [{name: "out material 1"}, {name: "out material 2"}];

        MockedMaterialControllerApi.getMaterialsByCompany.mockReturnValue(Promise.resolve(outputMaterials));
        ...
        expect(MockedFormGroup).toHaveBeenCalledTimes(26);
        expect(MockedFormLabel).toHaveBeenCalledTimes(26);
        expect(MockedFormLabel).toHaveBeenNthCalledWith(1, {children: 'certification.entity'}, {});
        ...
        expect(MockedFormLabel).toHaveBeenNthCalledWith(8, {children: 'materials (output)'}, {});
        expect(MockedFormLabel).toHaveBeenNthCalledWith(9, {children: 'certification.process_types'}, {});
        ...
        expect(MockedFormLabel).toHaveBeenNthCalledWith(13, {children: 'notes'}, {});

        expect(MockedCertificationAttachment).toHaveBeenCalledTimes(2);
        expect(MockedCertificationAttachment).toHaveBeenNthCalledWith(2, {
            register: expect.any(Function),
            fileUploaded: documentUploaded,
            setFileUploaded: setDocumentUploaded
        }, {});

        expect(MockedSelect).toHaveBeenCalledTimes(16);
        expect(MockedSelect).toHaveBeenNthCalledWith(9, {
        ...
        expect(MockedSelect).toHaveBeenNthCalledWith(11, {
            value: null,
            onChange: expect.any(Function),
        ...
        expect(MockedSelect).toHaveBeenNthCalledWith(13, {
            value: null,
            onChange: expect.any(Function),
            options: assessmentTypes.map(a => ({value: a, label: a.name}))
        }, {});
        expect(MockedSelect).toHaveBeenNthCalledWith(14, {
            ...
            isMulti: true,
            placeholder: "placeholders.certification.product_categories",
            onChange: expect.any(Function),
            options: productCategories.map(p => ({value: p, label: p.name}))
        }, {});
        expect(MockedSelect).toHaveBeenNthCalledWith(15, {
            value: null,
            isMulti: true,
            onChange: expect.any(Function),
            placeholder: "placeholders.certification.material",
            options: outputMaterials.map(m => ({value: m, label: m.name}))
        }, {});
        expect(MockedSelect).toHaveBeenNthCalledWith(16, {
            value: processesTypeSelected.map(p => {
                return {
                    value: p,
                    label: p?.name
                };
            ...
            isMulti: true,
            onChange: expect.any(Function),
            options: processTypes.map(p => ({value: p, label: p.name}))
        }, {});
    ...
    it('handleCertification test', async () => {
        ...
        act(() => {
            MockedSelect.mock.calls[8][0].onChange({value: {companyName: 'companyNameTest'}})
        });
        //Set consigneeEmail
        act(() => {
            MockedSelect.mock.calls[9][0].onChange({value: {companyName: 'test@test.ch'}})
        });
        //Set certificationTypeCode
        act(() => {
            MockedSelect.mock.calls[10][0].onChange({value: {code: 'certificationTypeCodeTest'}})
        });
        //Set processingStandardName
        act(() => {
            MockedSelect.mock.calls[11][0].onChange({value: {name: 'processingStandardName'}})
        });
        //Set assessmentName
        act(() => {
            MockedSelect.mock.calls[12][0].onChange({value: {name: 'assessmentNameTest'}})
        });
        //Set productCategoryCodeList
        act(() => {
            MockedSelect.mock.calls[13][0].onChange([{value: {code: 'productCategoryCode1Test'}}, {value: {code: 'productCategoryCode2Test'}}])
        });
        //Set outputMaterials
        act(() => {
            MockedSelect.mock.calls[14][0].onChange([{value: {name: 'outMaterialName1'}}, {value: {name: 'outMaterialName2'}}])
        });
        //Set processTypeCodeList
        act(() => {
            MockedSelect.mock.calls[15][0].onChange([{value: {code: 'processTypeCode1Test'}}, {value: {code: 'processTypeCode2Test'}}])
        });
    ...
    it('handleCertification failed due to wrong document URL', async () => {
        ...
        await act(async () => {
            component = mount(<ScopeCertificationInsert
                selectCompany={selectCompany} companySelected={companySelected} consigneeEmailSelected={consigneeEmailSelected}
                getUserFromEmailAddress={getUserFromEmailAddress} companyEmailAddresses={companyEmailAddresses}
                getMaterialsByCompany={getMaterialsByCompany}
                checkValidUntilDate={checkValidUntilDate}
