...
import ScopeCertificationConfirm from "../../confirm/ScopeCertificationConfirm/ScopeCertificationConfirm";
import {CertificationConfirmation} from "../../confirm/CertificationConfirmation";
import ScopeCertificationView from "../../view/ScopeCertificationView/ScopeCertificationView";
...
export const ScopeCertificationHandler = (props: Props) => {
    const { t } = useTranslation();
    const match = useRouteMatch('/:companyIndustry' + SCOPE_CERTIFICATION_CONFIRMATION_PATH);
    ...
    const isViewOrConfirmation = async () => {
        ...
        try {
            props.startLoading(t("popups.loading.scope_certification"));
            ...
            setIsView(true);
            setIsLoading(false);
            props.addErrorMessage(`${t("popups.errors.scope_certification")}: ${error}`);
        } finally {
            props.stopLoading();
        }
    };
    ...
    if (!isLoading){
        if (isView) {
            return (
                <CertificationViewer component={ScopeCertificationView} certification={certification} />
            );
        }
        return (
            <CertificationConfirmation component={ScopeCertificationConfirm} certification={certification} />
        );
    }
...
export default connector(ScopeCertificationHandler);
