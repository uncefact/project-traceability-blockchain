...
import MCH, {SelfCertificationHandler} from "./SelfCertificationHandler";
...
describe('Self certification handler test', () => {
    ...
    it('Content test', async () => {
        const certificationRetrieved : ConfirmationCertificationPresentable = {
            ...
            contractorEmail: "test@mail.ch"
        };

        let component : any = null;

        await act(async () => {
            component = await mount(<SelfCertificationHandler
                addErrorMessage={addErrorMessage} addSuccessMessage={addSuccessMessage}
                startLoading={startLoading} stopLoading={stopLoading}
                userLoggedIn={userLoggedIn} />
            )});
        ...
        await act(async () => {
            await mount(<SelfCertificationHandler
                addErrorMessage={addErrorMessage} addSuccessMessage={addSuccessMessage}
                startLoading={startLoading} stopLoading={stopLoading}
                userLoggedIn={userLoggedIn} />
            )});
        ...
        MockedCertificationControllerApi.getCertification.mockReturnValue(Promise.resolve(certificationRetrieved));
        await act(async () => {
            mount(<SelfCertificationHandler
                addErrorMessage={addErrorMessage} addSuccessMessage={addSuccessMessage}
                startLoading={startLoading} stopLoading={stopLoading}
                userLoggedIn={userLoggedIn} />
            )});
        ...
        // redirect to certification view due to user's permission (approver user)
        MockedCertificationConfirmation.mockClear();
        MockedCertificationViewer.mockClear();
        ...
        // redirect to certification view due to user's permission (the user can't be the one that has inserted the certification)
        MockedCertificationConfirmation.mockClear();
        MockedCertificationViewer.mockClear();
        certificationRetrieved.approverName = userLoggedIn.company.companyName;
        certificationRetrieved.contractorEmail = userLoggedIn.email;
        certificationRetrieved.consigneeEmail = userLoggedIn.email;
        MockedCertificationControllerApi.getCertification.mockReturnValue(Promise.resolve(certificationRetrieved));
        await act(async () => {
            mount(<SelfCertificationHandler
                addErrorMessage={addErrorMessage} addSuccessMessage={addSuccessMessage}
    ...
    it('Certification loading failed', async () => {
        ...
        expect(addErrorMessage).toHaveBeenNthCalledWith(1, "popups.errors.self_certification: Generic error");
    });
