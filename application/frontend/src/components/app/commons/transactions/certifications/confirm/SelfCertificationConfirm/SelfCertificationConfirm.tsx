...
export const SelfCertificationConfirm = (props: Props) => {
    const history = useHistory();
    // @ts-ignore
    const idParam = useRouteMatch('/:companyIndustry' + SELF_CERTIFICATION_CONFIRMATION_PATH)?.params.id;
    ...
    const handleConfirmation = async (isConfirmed: boolean) => {
        ...
        try {
            ...
            props.addSuccessMessage(t("popups.success.self_cert_add"));
            history.push("/");
        } catch (error) {
    ...
    return (
        <Jumbotron className={styles.Container}>
            <h2>{t("certification.confirmation.self_title")}</h2>
...
export default connector(SelfCertificationConfirm);
