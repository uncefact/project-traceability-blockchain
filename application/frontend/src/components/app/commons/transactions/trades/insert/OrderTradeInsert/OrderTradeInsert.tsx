...
import {TradeInsertionChildProps} from "../../../../../commons/transactions/trades/insert/TradeInsertion";
...
export const OrderTradeInsert = (props: Props) => {
    const history = useHistory();
    const { register, handleSubmit, errors, getValues } = useForm<OrderRequest>();
    const [documentSelected, setDocumentSelected] = React.useState<{value: DocumentTypePresentable | undefined, label: string}>();
    ...
    const handleTransaction = async (transaction: OrderRequest) => {
        setSubmitted(true);
        if (props.companySelected && documentSelected) {
            if (transaction.positions) {
                transaction.documentTypeCode = documentSelected.value?.code;
                ...
                try {
                    await TransactionControllerApi.createOrder({
                        orderRequest: transaction
                    });
                    props.addSuccessMessage(t("popups.success.order_create"));
                    history.push("/");
                } catch (error) {
    ...
    const getOrderDocumentTypes = async () => {
        try {
    ...
    return <Jumbotron className={styles.Container}>
        ...
        <Form className={styles.Form} onSubmit={handleSubmit(handleTransaction)}>
            ...
            <Form.Group className={styles.DocumentTypeArea}>
                <Form.Label>{t("trade.order_type")}</Form.Label>
                <Select
                ...
                />
                {
                    submitted && documentSelected === undefined &&
                    <div className={styles.ErrorText}>{t("errors.trade.order_type")}</div>
                }
            </Form.Group>
            ...
            <Form.Group className={styles.TradeReferenceArea}>
                <Form.Label>{t("trade.order_id")}</Form.Label>
                <Form.Control name="contractorReferenceNumber" ref={register({required: true})} type="text" placeholder={t("placeholders.trade.order_id")}/>
                <Form.Text className="text-muted">
                    i.e. GC14LU3
                </Form.Text>
                {
                    errors.contractorReferenceNumber && errors.contractorReferenceNumber.type === "required" &&
                    <div className={styles.ErrorText}>{t("errors.trade.order_id")}</div>
                }
            </Form.Group>
            <Form.Group className={styles.ParentTradeReferenceArea}>
                <Form.Label>{t("trade.order_root_id")}</Form.Label>
                <Form.Control name="contractorRootReferenceNumber" ref={register({required: false})} type="text" placeholder={t("placeholders.trade.order_root_id")}/>
                <Form.Text className="text-muted">
                    i.e. GC14LU3
                </Form.Text>
                {
                    errors.contractorRootReferenceNumber && errors.contractorRootReferenceNumber.type === "required" &&
                    <div className={styles.ErrorText}>{t("errors.trade.order_root_id")}</div>
                }
            </Form.Group>
...
export default connector(OrderTradeInsert);
